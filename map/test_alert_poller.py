...
def test_fetch_problems_serializes_host_and_severity_label(zabbix_client, requests_mock):
    requests_mock.post(API, [
        ...
        {'json': {'jsonrpc': '2.0', 'id': 3, 'result': [{
            ...
            'acknowledged': '0', 'hosts': [{'hostid': '10', 'name': 'DTC - CARACOL - 6'}],
        }]}},
    ])
...
def test_fetch_problems_sorts_by_severity_then_recency(zabbix_client, requests_mock):
    requests_mock.post(API, [
        _login_response(),
        {'json': {'jsonrpc': '2.0', 'id': 3, 'result': [
            {'eventid': '1', 'name': 'old high', 'severity': '4', 'clock': '100', 'acknowledged': '0', 'hosts': []},
            {'eventid': '2', 'name': 'disaster', 'severity': '5', 'clock': '50', 'acknowledged': '0', 'hosts': []},
            {'eventid': '3', 'name': 'new high', 'severity': '4', 'clock': '200', 'acknowledged': '0', 'hosts': []},
        ]}},
    ])
...
def test_fetch_problems_defaults_host_to_dash_when_missing(zabbix_client, requests_mock):
    requests_mock.post(API, [
        _login_response(),
        {'json': {'jsonrpc': '2.0', 'id': 3, 'result': [{
            'eventid': '9', 'name': 'orphan trigger', 'severity': '2', 'clock': '1', 'acknowledged': '1', 'hosts': [],
        }]}},
    ])
